...
function solution(roman) {
  ...
  for (const [i, letter] of romanArr.entries()) {
    if (romans[letter] > romans[romanArr[i - 1]])
      sum -= 2 * romans[romanArr[i - 1]];
    sum += romans[letter];
  }
  return sum;
}
...
console.log(solution("L"), 50);
console.log(solution("X"), 10);
console.log(solution("V"), 5);
